...
from config import train_hard_negative_config as train_config
...
def train(epoch,train_loader,model_main,loss_function,optimizer,lr_scheduler,log, model_momentum, queue_features, queue_labels):
    ...
    for idx,batch in enumerate(train_loader):
        ...
        if "ihc" in log.param.dataset or "hateval" in log.param.dataset or 'dynahate' in log.param.dataset or 'toxigen' in log.param.dataset or 'sbic' in log.param.dataset:
            text_name = "post"
            label_name = "label"
        else:
        ...
        if log.param.w_aug: # text split
            if log.param.w_double:
                ...
                if log.param.loss_type == "SemiCon":
                    ...
                    if log.param.loss_type not in ["CE", "SCL", "SupConLoss_Original", "AugCon"]:
                        with torch.no_grad():
                    ...
                    if log.param.loss_type not in ["CE", "SCL", "SupConLoss_Original"]:
                        with torch.no_grad():
    ...
    with torch.no_grad():
        for idx,batch in enumerate(test_loader):
            if "ihc" in log.param.dataset:
                ...
                text_name = "post"
                label_name = "label"
            elif "toxigen" in log.param.dataset or "hateval" in log.param.dataset:
                text_name = "post"
                label_name = "label"
            else:
                text_name = "cause"
